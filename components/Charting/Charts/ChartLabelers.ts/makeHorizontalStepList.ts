...
const makeHorizontalStepList: Signature = (
  ...
  const stepList = [Number(makeNumberReadable({ number: chartBottom }))];
  ...
  stepList.push(Number(makeNumberReadable({ number: chartTop }))); // We want the exact maximum value to be the final step, not just the last step we'd reach by stepping
